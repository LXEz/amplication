...
const service = {
  ...
  toDoTasks: () => FIND_MANY_RESULT,
  toDoTask: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("ToDoTask", () => {
  ...
  test("GET /toDoTasks/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/toDoTasks"}/${nonExistingId}`)
  ...
  test("GET /toDoTasks/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/toDoTasks"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /toDoTasks existing resource", async () => {
